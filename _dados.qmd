...
//| output: false
  ...
  filtered = dados_matriculas?.sheet?.("números absolutos")?.filter?.(filtro =>
    filtro?.A != "Territórios BA" && filtro?.B == id_pesquisa
  ) ?? [];

  filtered_distorcao = dados_taxas?.sheet?.("Taxa distorção")?.filter?.(filtro =>
    filtro?.A != "Territórios BA" && filtro?.B == id_pesquisa && (pesquisa_ano == "2010" || pesquisa_ano == "2020")
  ) ?? [];

  filtered_distorcao_anuais = dados_taxas?.sheet?.("Distorção anuais")?.filter?.(filtro =>
    filtro?.A != "Territórios/BA" && filtro?.B == id_pesquisa && filtro?.C != "Total" && (parseInt(pesquisa_ano) >= 2010) && (parseInt(pesquisa_ano) <= 2022)
  ) ?? [];

  filtered_rendimento = dados_taxas?.sheet?.("Taxa rendimento")?.filter?.(filtro =>
    filtro?.A != "Territórios BA" && filtro?.B == id_pesquisa && (pesquisa_ano == "2010" || pesquisa_ano == "2019")
  ) ?? [];

  filtered_rendimento_anuais = dados_taxas?.sheet?.("Rendimento anuais")?.filter?.(filtro =>
    filtro?.A != "Territórios/BA" && filtro?.B != "Total" && filtro?.C == id_pesquisa && (parseInt(pesquisa_ano) >= 2010) && (parseInt(pesquisa_ano) <= 2021)
  ) ?? [];

  aprovacoes_anuais = dados_taxas?.sheet?.("Rendimento anuais")?.filter?.(filtro =>
    filtro?.A != "Territórios/BA" && filtro?.B != "Total" && !isNaN(filtro?.C) && (parseInt(pesquisa_ano) >= 2010) && (parseInt(pesquisa_ano) <= 2021)
  )?.map?.(filtro => {
  ...
  aprovacoes_ordenadas = aprovacoes_anuais?.sort?.((a, b) => (b?.aprovacao ?? 0) - (a?.aprovacao ?? 0)) ?? [];

  reprovacoes_anuais = dados_taxas?.sheet?.("Rendimento anuais")?.filter?.(filtro =>
    filtro?.A != "Territórios/BA" && filtro?.B != "Total" && !isNaN(filtro?.C) && (parseInt(pesquisa_ano) >= 2010) && (parseInt(pesquisa_ano) <= 2021)
  )?.map?.(filtro => {
  ...
  reprovacoes_ordenadas = reprovacoes_anuais?.sort?.((a, b) => (a?.reprovacao ?? 0) - (b?.reprovacao ?? 0)) ?? [];

  abandono_anuais = dados_taxas?.sheet?.("Rendimento anuais")?.filter?.(filtro =>
    filtro?.A != "Territórios/BA" && filtro?.B != "Total" && !isNaN(filtro?.C) && (parseInt(pesquisa_ano) >= 2010) && (parseInt(pesquisa_ano) <= 2021)
  )?.map?.(filtro => {
  ...
  abandono_ordenadas = abandono_anuais?.sort?.((a, b) => (a?.abandono ?? 0) - (b?.abandono ?? 0)) ?? [];
  ...
  total_masculina_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Masculina ?? 0);

  total_feminina = dados?.[0]?.[pesquisa_ano]?.Feminina?.toLocaleString?.('pt-BR') ?? "-";
  total_feminina_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Feminina ?? 0);

  total_naodeclarada = dados?.[0]?.[pesquisa_ano]?.Nãodeclarada?.toLocaleString?.('pt-BR') ?? "-";
  total_naodeclarada_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Nãodeclarada ?? 0);

  total_branca = dados?.[0]?.[pesquisa_ano]?.Branca?.toLocaleString?.('pt-BR') ?? "-";
  total_branca_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Branca ?? 0);

  total_preta = dados?.[0]?.[pesquisa_ano]?.Preta?.toLocaleString?.('pt-BR') ?? "-";
  total_preta_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Preta ?? 0);

  total_parda = dados?.[0]?.[pesquisa_ano]?.Parda?.toLocaleString?.('pt-BR') ?? "-";
  total_parda_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Parda ?? 0);

  total_amarela = dados?.[0]?.[pesquisa_ano]?.Amarela?.toLocaleString?.('pt-BR') ?? "-";
  total_amarela_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Amarela ?? 0);

  total_indigena = dados?.[0]?.[pesquisa_ano]?.Indígena?.toLocaleString?.('pt-BR') ?? "-";
  total_indigena_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Indígena ?? 0);

  total_deficiencia = dados?.[0]?.[pesquisa_ano]?.Deficiência?.toLocaleString?.('pt-BR') ?? "-";
  total_deficiencia_porcentagem = calculateTotalPercentage(dados?.[0]?.[pesquisa_ano]?.Deficiência ?? 0);
